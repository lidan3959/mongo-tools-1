...
void _sync::cloneIndex(boost::shared_ptr<DBClientConnection> _s, 
                          boost::shared_ptr<DBClientConnection> _t, string db, string coll) {
  
  log() << "clone index: " << db << "." << coll << "\t to \t" << db << "." << coll << endl;
  
  BSONObj rst, rst_cursor, rst_batch, tmp;
  _s->runCommand(db , BSON("listIndexes" << coll) , rst);
  rst_cursor = rst.getObjectField("cursor");
  rst_batch = rst_cursor.getObjectField("firstBatch");
  int i = 0;
  char buf[10];
  string num;
  string rst_name;
//  _t->runCommand(db, BSON("createIndexes" << "fuck" << "indexes" << BSON_ARRAY(BSON("key" << BSON("f2" << 1) << "name" << "f2_1"))), tmp);
  while (true) {
    sprintf(buf, "%d", i);
    num = buf;
    if (rst_batch.hasField(num)) {
      _t->runCommand(db, BSON("createIndexes" << coll << "indexes" << BSON_ARRAY(rst_batch.getObjectField(num.c_str()))), tmp);
      i++;
    } else {
      break;
    }
...
//  string nsIndex = db + ".system.indexes";
  ...
  BSONObj rst, rst_cursor, rst_batch;
  _s->runCommand(db , BSON("listCollections" << 1) , rst);
  rst_cursor = rst.getObjectField("cursor");
//  string rst_ns = rst_cursor["ns"].String();
//  std::cout << rst_ns << std::endl;
  rst_batch = rst_cursor.getObjectField("firstBatch");
  int i = 0;
  char buf[10];
  string num;
  ...
  while (true) {
    ...
    i++;
    if (rst_batch.hasField(num)) {
      rst_name = (rst_batch.getObjectField(num.c_str()))["name"].String();
//      std::cout << rst_name << std::endl;
      if (_coll != "" && _coll != rst_name) {
        continue;
      }
      collections.push_back(db + "." + rst_name);
    } else {
      break;
    }
  }
...
//  }
  ...
  for (vector<string>::iterator it = collections.begin(); it != collections.end(); ++it) {
      string name = *it;
      log() << "clone collection: " << name << "\t to \t " << name << endl;
  ...
  if (_coll.empty()) {
    for (vector<string>::iterator it = collections.begin(); it != collections.end(); ++it) {
      ...
      cloneIndex(_s, _t, db, name);
    }
  } else {
    cloneIndex(_s, _t, db, _coll);
  }
