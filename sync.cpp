...
OpTime _sync::getToopStart(boost::shared_ptr<DBClientConnection> _s, boost::shared_ptr<DBClientConnection> _t, string strNs) {
    BSONObj objDst = _t->findOne(_toopNs, Query(BSON("ns" << BSON("\$regex" << strNs))).sort("$natural", -1), 0, QueryOption_SlaveOk);
    if (!objDst.binaryEqual(BSONObj())) {
        return objDst["ts"]._opTime();
    }
    BSONObj objSrc = _s->findOne(oplogName, Query().sort("$natural", 1), 0, QueryOption_SlaveOk);
    return objSrc["ts"]._opTime();
}
...
bool _sync::getAllCollections(string db, vector<string>& collections, boost::shared_ptr<DBClientConnection> _conn, string mongoVersion) {
    BSONObj tmp, collArray;
    if (mongoVersion.substr(0, 4) == "3.0.") {
        bool ok = _conn->runCommand(db, BSON("listCollections" << 1), tmp);
        if (!ok) {
            log() << "get listCollections failed" << endl;
            return false;
        }
        collArray = tmp.getObjectField("cursor").getObjectField("firstBatch");
        int idx = 0;
        while (collArray.hasField(boost::lexical_cast<string, int>(idx))) {
            collections.push_back(db + "." + collArray.getObjectField(boost::lexical_cast<string, int>(idx++).c_str()).getStringField("name"));
        }
    } else if (mongoVersion.substr(0, 4) == "2.4.") {
        auto_ptr<DBClientCursor> c = _conn->query(db + ".system.namespaces" , Query() , 0 , 0 , 0 , QueryOption_SlaveOk | QueryOption_NoCursorTimeout);
        string coll;
        while (c->more()) {
            tmp = c->nextSafe(); 
            coll = tmp.getStringField("name");
            if (endsWith(coll.c_str(), ".system.indexes") || endsWith(coll.c_str(), ".system.users") || coll.find(".$") != string::npos || coll.find_first_of("/\0") != string::npos) {
                continue;
            }
            collections.push_back(coll);
        }
    } else {
        log() << "mongo version " << mongoVersion << " not supported" << endl;
        return false;
    }
    return true;
}

void _sync::cloneCollectionIndex(boost::shared_ptr<DBClientConnection> _s,
        boost::shared_ptr<DBClientConnection> _t, string collFullname, int* total) {
    BSONObj tmp, indexesArray;
    BSONArrayBuilder objArrayBuilder;
    NamespaceString ns(collFullname);
    auto_ptr<DBClientCursor> c(NULL);
    if (_srcVersion.substr(0, 4) == "3.0.") {
        bool ok = _s->runCommand(ns.db, BSON("listIndexes" << ns.coll), tmp);
        if (!ok) {
            log() << collFullname <<  " get listIndexes failed" << endl;
            return;
        }
        indexesArray = tmp.getObjectField("cursor").getObjectField("firstBatch");

    } else if (_srcVersion.substr(0, 4) == "2.4.") {
        c = _s->query(ns.db + ".system.indexes", Query(BSON("ns" << collFullname)), 0, 0, 0, QueryOption_SlaveOk | QueryOption_NoCursorTimeout);
        while (c->more()) {
            tmp = c->next();
            objArrayBuilder << tmp;
        }
        indexesArray = objArrayBuilder.arr();
    } else {
        log() << "the source mongo version is not surpported" << endl;
        return;
    }
    *total += indexesArray.nFields();


    int idx = 0;
    string idxStr;
    while (indexesArray.hasField(idxStr = boost::lexical_cast<string, int>(idx++))) {
        if (_dstVersion.substr(0, 4) == "3.0.") {
            _t->runCommand(ns.db, BSON("createIndexes" << ns.coll << "indexes" << BSON_ARRAY(indexesArray.getObjectField(idxStr.c_str()))), tmp); // not check the operation's successness
        } else if (_dstVersion.substr(0, 4) == "2.4.") {
            _t->insert(ns.db + ".system.indexes", indexesArray.getObjectField(idxStr.c_str()));
        } else {
            log() << "the destination mongo version is not surpported" << endl;
        }
    }
    return;
}

void _sync::cloneDbIndex(boost::shared_ptr<DBClientConnection> _s,
        boost::shared_ptr<DBClientConnection> _t, string db) { // this is just used by clonedb, not for cloneCollection
    log() << "clone " << db << " index" << endl;
    BSONArrayBuilder objArrayBuilder;
    vector<string> collections;
    int total = 0;
    bool ok = getAllCollections(db, collections, _s, _srcVersion);
    if (!ok) {
        log() << "getAllCollections failed" << endl;
        return;
    }
    for (vector<string>::iterator iter = collections.begin(); iter != collections.end();
            ++iter) {
        cloneCollectionIndex(_s, _t, *iter, &total);
    }
    log() << "\t\t" << total << " objects" << endl;
}
...
void _sync::batchCloneCollection(boost::shared_ptr<DBClientConnection> _s,
    ...
    log() << "cloning " << sns << " -> " << tns << endl;
    int queryOptions = QueryOption_SlaveOk | QueryOption_NoCursorTimeout;
    unsigned long long n = _s->count(sns, _query, queryOptions);
    ProgressMeter m(n);
    m.setName("Cloning Progress");
    m.setUnits("objects");
    ProgressMeter* _m=&m;
...
void _sync::clonedb(boost::shared_ptr<DBClientConnection> _s,
    ...
    BSONObj collsObj, collsObjSub;
    vector <string> collections;
    int queryOptions = QueryOption_SlaveOk | QueryOption_NoCursorTimeout, index = 0;
    ...
    if (_srcVersion.substr(0, 4) == "3.0.") {
        bool ok = _s->runCommand(db, BSON("listCollections" << 1), collsObj);
        if (!ok) {
            log() << "get " << db << "'s collections failed" << endl;
            return;
        }
        collsObjSub = collsObj.getObjectField("cursor").getObjectField("firstBatch");
        while(collsObjSub.hasField(boost::lexical_cast<std::string, int>(index))) {
            collName = collsObjSub.getObjectField(boost::lexical_cast<string, int>(index).c_str()).getStringField("name");
            index++;
            collections.push_back(db + "." + collName);
        }
    } else if (_srcVersion.substr(0, 4) == "2.4.") {
        while ( cursor->more() ) {
        ...
        log() << "ERROR: mongo version " << _srcVersion << " is not supported" << endl;
        return;
    }
...
void _sync::go(boost::shared_ptr<DBClientConnection> _s,
    ...
    if (!hasParam("db")) {  // all dbs
        if (!hasParam("optimestart") && !_onlyOplog)  {  // no optimestart,clone all dbs
        ...
        if (hasParam("oplog") || _onlyOplog) {
            if (hasParam("optimestart") && hasParam("optimestop")) {
                ...
                if(isStale(_s, lastOp, remoteOldestOp)) {
                    ...
                    syncAllGTE(_s, _t, _onlyOplog ? getToopStart(_s, _t, "") : lastTs);
                }
            }
        }
        ...
        if (hasParam("tns"))
            ...
            tns = sns;
        if (!hasParam("optimestart") && !_onlyOplog) {  // optimestart is only for incremental oplog sync
            // clone data
            if (hasParam("tns"))
        ...
        if (hasParam("oplog") || _onlyOplog) {
            ...
            if (hasParam("optimestart") && hasParam("optimestop")) {
                ...
                if ( hasParam("collection"))
                    syncCollection(_s, _t, sns, tns, _onlyOplog ? getToopStart(_s, _t, "^" + NamespaceString(sns).db + "\." + NamespaceString(sns).coll) : lastTs);
                else
                    syncdb(_s, _t, _db, _onlyOplog ? getToopStart(_s, _t, "^" + NamespaceString(sns).db + "\." + NamespaceString(sns).coll) : lastTs);
            }
        }
    }
}
...
string _sync::getMongoVersion(boost::shared_ptr<DBClientConnection> _c) {
    BSONObj buildInfo;
    string mongoVersion;
    _c->simpleCommand("admin", &buildInfo, "buildInfo");
    if (buildInfo["version"].trueValue()) {
        mongoVersion = buildInfo.getStringField("version");
    }
    return mongoVersion;
}
...
int _sync::run() {
    ...
    if (hasParam("only-oplog")) {
        _onlyOplog = true;
    } else {
        _onlyOplog = false;
    }
    if (hasParam("toopns")) {
        _toopNs = getParam("toopns");
    } else {
        _toopNs = "sync.oplog";
    }
    _player->SetOnlyOplog(_onlyOplog);
    _player->SetToopNs(_toopNs);
    ...
    _srcVersion = getMongoVersion(_s);
    _dstVersion = getMongoVersion(_t);
